...
// --- CHECKLIST ROUTES ---
router.get('/checklist/categories', async (req, res) => {
    try {
    ...
    try {
        const TECHNICAL_CHECKLIST = [
            ...
            },
            {
              ...
              items: [
                ...
                { id: '3.2', text: 'Há sinalização de advertência e isolamento ao redor das escavações?' },
        ...
        for (let i = 0; i < TECHNICAL_CHECKLIST.length; i++) {
            ...
            for (let j = 0; j < cat.items.length; j++) {
                ...
                const existing = await prisma.checklistItem.findFirst({ where: { code: item.id, categoryId: category.id } });
                if (existing) {
                    await prisma.checklistItem.update({ where: { id: existing.id }, data: { text: item.text, order: j } });
                } else {
                    await prisma.checklistItem.create({ data: { categoryId: category.id, code: item.id, text: item.text, order: j } });
                }
            }
        }
        res.json({ message: 'Checklist semeado com sucesso!' });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Erro ao semear checklist' });
    }
});
